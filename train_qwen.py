...
class QwenAdaptiveLatentReasoning(nn.Module):
...
def main():
    ...
    print(f"Deploying Qwen-Instruct Overdrive on {device}...")
    ...
    model = QwenAdaptiveLatentReasoning(base_model).to(device)
    optimizer = AdamW(model.parameters(), lr=2e-5)

    dpo_data = []
    with open("data/dpo_train_qwen.jsonl", "r") as f:
        for line in f:
            dpo_data.append(json.loads(line))

    model.train()
    # THE FIX: 4 Epochs forces the model to memorize the latent routing rules
    epochs = 4 
    beta = 0.1
    
    print(f"\n>> Commencing Deep LoRA DPO Training ({epochs} Epochs)...")
    ...
    model.base_model.save_pretrained("saved_models/qwen_adaptive_lora")
    torch.save(model.halt_head.state_dict(), "saved_models/qwen_halt_head.pt")
    print(">> Overdrive Weights Saved.")
